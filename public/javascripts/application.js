...
var markers;
var epsg4326;
var google_api_key = 'ABQIAAAApzvKd8OI-7IEJPl_AD0lOhRgdPDzh_uHubaZN8_GGD4D_cXeKBTTeEnRoQi67FtmWS_8Q6Z6chPEgg';
...
$(document).ready(function() {
  ...
  if ($('div.twitter')[0]) {
    ...
    $.getJSON(url,function(data){
      var dataSize = parseInt(data.length) - 1;
      $.each(data, function(i, item) {
        var str = "<li><div class='image'><img src=\""+item.user.profile_image_url+"\" alt='"+item.user.name+"' title='"+item.user.name+"'/></div><div class='text'><p>"+replaceURLWithHTMLLinks(item.text)+"</p></div></li>";
        if (i == dataSize) {
          var str = "<li class='last'><div class='image'><img src=\""+item.user.profile_image_url+"\" alt='"+item.user.name+"' title='"+item.user.name+"'/></div><div class='text'><p>"+replaceURLWithHTMLLinks(item.text)+"</p></div></li>";
        }
